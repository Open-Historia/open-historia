...
**Every call starts at the top again, except past a Spent entry.** A short mark — busy, rate limited — says what a Settings row shows and what the "nothing can answer" message says; it never takes an entry out of a call's order. The strongest model is asked first every time, so a key that was topped up or a busy spell that passed is used the moment it can answer, at the cost of one fast failed request (a 404 or a 429 is refused in well under a tenth of a second; see `.lab/probes/fallback-timing-probe.mjs`).
