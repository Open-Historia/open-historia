...
| **[Map Truth — Control is not Sovereignty]** + **[Current Non-Normal Territorial State]** — capture/occupation/liberation language *requires* `impacts.regionControlOps` (control / contest), and a side that is not a state says what it is in `actorRole` / `toRole`; cession, annexation, sale, unification or settlement *requires* `impacts.regionTransfers`; the current occupations and contested regions (`territorialControlContext`) are listed, followed by what each claimant named there is (`What these claimants are:`, from its record's `role`); resolving the player's own ordered offensives is allowed | `jumpForward`, `autoJumpForward` | `gameplay.js` |
