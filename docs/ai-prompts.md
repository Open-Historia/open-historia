...
- **`worldMotionRepair`** (tool `submit_world_motion_repair`, `WORLD_MOTION_REPAIR_SCHEMA`): after a segment is accepted, a selected storyline that is objectively stale past its anti-stasis backstop, or whose update was omitted, gets one narrow repair call (`runTargetedWorldMotionRepair`: inline system prompt, no events or other ledgers, `callRepairAI`). A failed repair leaves the process overdue for the next turn; it never costs the turn. Past the anti-stasis backstop (`storylineAtAntiStasisBackstop`) the prompt states the validator's numeric rule — change status, or move pressure ≥`ANTI_STASIS_MIN_PRESSURE_DELTA` (4) or momentum ≥`ANTI_STASIS_MIN_MOMENTUM_DELTA` (6) — because a prose-only change is rejected there. **Limits** (`nativeWorldDirector.js`, budget per skip in `jumpState.motionRepairBudget`): each storyline is repaired at most once per skip, however many segments re-flag it; a skip makes at most `MAX_MOTION_REPAIRS_PER_JUMP` (8) repair calls and starts none after `MAX_MOTION_REPAIR_MS_PER_JUMP` (10 min); a storyline whose repair failed is not retried for `MOTION_REPAIR_FAILURE_COOLDOWN_ROUNDS` (3) rounds unless it changes (in-memory `motionRepairFailures`, keyed by campaign). A skipped issue is handled exactly like a failed repair, and each pass logs one `[OH World Motion Repair]` summary line (attempted, repaired, failed, left overdue and why).
