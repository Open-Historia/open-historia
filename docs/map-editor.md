...
| **Disputed by** (claimants) | `{ claimants }` + each claimant's `role` | `TagField` of claimant names — a country, a movement, a gang, one side of a civil war: any name. Any claimant makes the region render **striped** (owner colour + each claimant's), here and in-game. Under the field, **What each claimant is** gives every claimant a line of the author's own words ("a country claiming this land as its own", "a terrorist organisation", "the rebel side of the civil war"). It is stored as that polity's `role` in the registry (`upsertPolity`; a claimant with no record yet gets one, as the Countries panel would make it), so it belongs to the claimant, not the region: the same line shows under every region it claims and in the Countries panel, and the AI is shown it wherever that polity appears ([World state](world-state.md), `polityOverrides.role`). |
...
| `world.polityOverrides` | `{name:{name,aliases:[],color:'#hex',note:'',role?,verbatim?}}` — `role` is what the polity is, in the author's words, when one was written. |
...
**Export.** `buildGameSeed` emits one `polityOverrides` record per registry entry and per owner (code = stable key, display name, cumulative aliases, colour, status, `role` when the author wrote what it is, `verbatim` for a code-shaped key) plus `ownerSchema` and the map's disputes as `world.regionClaimants`, and cities export their authored `tier` (1 town, 2 city, 3 major) with `capital` as an independent flag (`CityPopup.jsx`).
