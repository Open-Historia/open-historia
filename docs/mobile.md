...
- 2026-09-26 (Android 15 emulator, the on-screen keyboard): the AI-key box, low on the setup screen, stays in view above the keyboard with its buttons (build 14: under the keyboard); on the country picker the keyboard leaves the card where it was, the map drawn, and typing folds the map away so the matches sit above the keys.
