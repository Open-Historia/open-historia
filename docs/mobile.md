...
| Update | `src/runtime/web/router.js` `app-update` | Reads `releases/download/android/latest.json`, which `android-apk.yml` writes (the beta: `android-beta/latest.json`, `VITE_APP_TRACK=beta`). `versionCode`/`versionName` come from the run number. |
| Beta app | `mobile/android/app/build.gradle` (`OH_ANDROID_CHANNEL`), `.github/workflows/android-apk-beta.yml` | A second app built from `beta`: "Open Historia Beta", `io.github.arkniem.paxhistoria.beta`, the compass with a BETA banner (`scripts/make-android-beta-icons.ps1`; the manifest takes its label and icons from placeholders the channel sets). It installs beside the stable app with its own saves and updates from the `android-beta` pre-release. The stable app is built from `main` only; each workflow refuses the other's branch. `server/androidBetaPackaging.test.js` keeps the pieces in step. |
...
CI: `.github/workflows/android-apk.yml` (`workflow_dispatch` from `main`, or an `android-v*` tag): `npm run build:android` → in `mobile/`: `npm ci`, `npm run map` (cached on the manifest's hash), `npm run www`, `npx cap sync android`, `./gradlew assembleRelease` with `OH_ANDROID_BUILD=<run number>` — signed with the `ANDROID_KEYSTORE_*` secrets when present, debug-signed otherwise — then publishes `open-historia.apk` and `latest.json` to the rolling `android` release. The beta app is the same steps in `android-apk-beta.yml` (`workflow_dispatch` from `beta`, or an `android-beta-v*` tag) with `OH_ANDROID_CHANNEL=beta` and `VITE_APP_TRACK=beta`, publishing `open-historia-beta.apk` and its own `latest.json` to the `android-beta` pre-release.
...
On 2026-09-26 the first CI-signed builds of the two apps were checked on the Android 15 emulator (stable build 15 from `main`, beta build 5 from `beta`): each app showed the update banner for its own release's `latest.json` (a stable build 14 offered build 15; a beta build 1 offered beta build 5), each published APK installed over the older build, and the stable app kept its saved game. Not checked yet: a physical phone, a turn with a real cloud key, the community hub.
