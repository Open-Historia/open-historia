...
| `groups` | `{ [name]: { name, description, color, formerNames?[] } }` (≤64) | `{}` | **Groups** (`src/runtime/groups.js`): actors that are not countries — a terrorist organisation, a cartel, a militia, a zombie outbreak — keyed by exact name like polities. `description` is what the AI is told the group is (≤1,200 chars); `color` is the `#rrggbb` tint of its area (a palette colour hashed from the name when none is given). Written by `impacts.groupOps`, the Workshop's Groups panel and the Groups GM tool. A rename keeps the old name in `formerNames`, so a model that still writes it reaches the group. |
| `groupAreas` | `{ [regionId]: groupName }` | `{}` | Which group CONTROLS each region: sparse, one group per region, a row naming no group is dropped. The region stays its owner's — the map outlines each group's whole area in its colour and tints it lightly (`GroupAreaLayers.jsx`, cut from the frontier topology by `vnext/groupAreas.js`), and the region card says **Group control**. |
...
| `markers` | `Marker[]` | `[]` | `normalizeMarkerEntry`: built structures — `{id,name,kind,ownerCode,lng,lat,note,foundedAt,createdAt}`. A scenario starts with the ones placed with the Workshop's Map feature tool (map-editor.md §9d), each with a stable id (a marker without one gets a new random id on every read). |
...
| `groupOps` | `normalizeGroupOp` (`groups.js`) → `{op: create\|update\|dissolve\|take\|release, name, newName?, description?, color?, regionIds[], note}` | `applyGroupOps` (`groups.js`), after the event's territory has moved | `create` founds a group; `update` changes its description, colour or name (re-keying its area); `take` puts regions in its area (taking them from any other group); `release` gives them back (all of them when `regionIds` is empty); `dissolve` erases it and its area. Lenient where the intent is plain: an `update` or `take` naming a group nobody has creates it; a `release` or `dissolve` of an unknown group does nothing. Moves no border. |
