...
| `polityOverrides` | `{ name: {code,name,aliases[],formerNames?[],color,note,role?,status?,mapRefs?{gadm0[]},mapLabel?,mapDistinctLabel?,verbatim?} }` | `{}` | Declared/renamed polities: new countries, renames, colors, alt-names. **A polity is keyed by its name and `name` equals the key; a rename re-keys the country everywhere** (`server/polityRename.js` `renamePolityInWorld`, reached from an event's `polityChanges` — a `rename`, or an `update` carrying a new `name` — and from the Workshop) with the old name kept in `formerNames`, which `buildOwnerAliasMap` folds onto the country and `resolveCountryTags` uses to find the scenario's starting tags. In every map keyed by names (colours, flags, stats, tags, reputation, intelligence, the standing goal) the country's own value moves to the new name and anything already under it is dropped: no other polity may hold that name, so it is a leftover, such as an unused stock-palette colour, and it must not repaint or restate the renamed country. A save whose record still shows a display name over a different key is re-keyed on its next apply (`displayNameMigrations`), except onto a real country's name that was never its own. Written by AI `polityChanges` and the editor. `enqueueContentStrings` translates names on write. Normalized by `normalizePolityOverride`. Feeds `loadCountryNames` (`assets.js`) and every name/flag resolver. **`role`** is what the polity is, in one line of free text — "a terrorist organisation", "the rebel side of the civil war", "a country claiming this land as its own" (`server/polityRole.js`, at most 240 characters). It is written in the Workshop (the Countries panel's **What it is**, and under the inspector's **Disputed by**) and by the AI (`regionClaims.claimantRole`, a contest's `actorRole`, a control's `toRole`, `polityChanges.role`; an empty value keeps the old one), and shown to the model beside the polity's exact name, never inside it: the claimant legend under the territorial context, the target dossier, the world roster, the lookups (`role`, `claimantRoles`, `participantRoles`), the territory director's state, the war ledger, the world director's pressure lines and a chat's participant list. The player sees it in the region popup, the country panel and the cheats' region inspector. |
| `regionClaimants` | `{ regionId: string[] }` (≤4) | `{}` | Marks a region DISPUTED — striped in the administrator's + claimants' colors. World-data equivalent of a `claimants` list on the geojson feature, and WINS over feature props. Normalized (sliced to 4). Sparse: a row only while a region is disputed. The world readers (`readWorldState`, `readWorldStateView`) add the map file's own `claimants` for every region with no row that is not settled (`withMapClaims`, `src/runtime/mapClaims.js`, from the primed region catalog), so the AI, the region popup and the cheats see the disputes the map draws; before, a dispute drawn in the Workshop striped the map and no prompt ever named it. |
...
|---|---|---|---|
| `regionControlOps` | `normalizeRegionControlOp` → `{op: contest\|control\|clear_contest, regionId, fromCode, actorCode\|toCode\|claimantCode, actorRole?\|toRole?, clearAll?, wholeCountry?, note}` | `applyPolityAndTerritoryImpacts` | De-facto control without a legal change: `contest` adds a contender to the stripes, `control` makes `toCode` the controller (the previous controller and the lawful sovereign become claimants, the sovereign anchored in `regionSovereigntyOverrides`), `clear_contest` removes one claimant (or all with `clearAll`, never the displaced sovereign). A contest's `actorRole` and a control's `toRole` — what that side is — are written to its record's `role`. Proposed by the simulator and by `nativeTerritoryDirector.js`; resolved through the same geography resolver as transfers, bounded by current control. |
| `regionClaims` | `normalizeRegionClaim` → `{regionId,claimantCode,claimantRole?,drop,note}` | `applyPolityAndTerritoryImpacts` | Appends to / removes from `regionClaimants[regionId]` (de-duplicated case-insensitively; the key is deleted at zero). Applied **before** `regionTransfers`, so a region claimed and handed over in the same jump ends settled rather than striped. Until this existed nothing but the map editor and the cheats panel could raise a dispute, so a unilateral claim had nowhere to go but a project. A claimant the world does not know is founded as a landless polity rather than written as a bare name. `claimantRole` — what the claimant is — is written to its record's `role` (a claimant known only from claimant lists gets a record for it; a dropped claim describes nobody). Plain region names are resolved to ids beside the transfers (`resolveRegionTransfers`); a claim that matches nothing is dropped, never failed. |
...
| Function | Line | Notes |
|---|---|---|
| `readWorldState({force})` | | `readJson(world)` → `normalizeWorldState` → `withMapClaims` (the map file's disputes for regions the world has no row for; `readWorldStateView` does the same). |
