...
// on every commit while `no-undef` never ran, and a missing React prop reached a
// player as a white screen ("projectsHeld is not defined"). That is exactly the
// class of bug linting exists to catch, so the glob now covers what the project
// is actually written in.
//
// The rules below are set to `warn` rather than `error` on purpose: they are
// pre-existing findings across 177 files, and turning them into errors would
...
export default defineConfig([
  globalIgnores(['dist', 'dist-web', 'release', 'server/data']),
  {
    files: ['**/*.{js,jsx,ts,tsx}'],
    extends: [
