...
test("a scenario stores a complete configuration and a game only its overrides", () => {
  ...
  `);
  assert.deepEqual(result.scenario, { espionage: { enabled: false }, idleDiplomacy: { enabled: true, averageMinutes: 30 }, worldDirection: WORLD_DIRECTION_DEFAULTS, playerFocus: PLAYER_FOCUS_DEFAULTS });
...
test("a save that does not mention features keeps them, and a fresh install reads defaults", () => {
  ...
  `);
  assert.deepEqual(result.before, { espionage: { enabled: true }, idleDiplomacy: { enabled: true, averageMinutes: 8 }, worldDirection: WORLD_DIRECTION_DEFAULTS, playerFocus: PLAYER_FOCUS_DEFAULTS });
