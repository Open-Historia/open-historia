/*!
 * Continuum Scenario Workshop - Province Map Importer
 * Continuum fork only.
 */
...
// Region ownership uses stable polity keys in current Continuum documents. Imported
...
const collectImportedPolities = (features, polities, flags, embeddedPolityRows = [], { materializeStandardFlags = true } = {}) => {
  ...
  if (conflicts.length) {
    throw new Error(
      ...
      "Continuum will not choose a colour based on feature order. Fix the source GeoJSON and analyze again. " +
...
const ProvinceImportPanel = ({ api, polities = {}, flags = {}, importPolityRoster, importCityMarkers, currentPointFeatures = [], onClose, onApplied }) => {
  ...
  const apply = async () => {
    ...
    const ok = window.confirm(
      ...
      "\n\nContinuum will automatically download a GeoJSON backup of the current regions and city markers first. " +
    ...
    >
      ...
      <div style={{ padding: 13, display: "grid", gap: 12 }}>
        <div style={{ fontSize: 12, lineHeight: 1.45, color: "rgba(255,255,255,0.72)" }}>
          Import a user-supplied colour-coded province raster (including a local HOI4-style <b>provinces.bmp</b>) and turn its colours into real Continuum regions. Shared borders are traced from one pixel lattice, so imported neighbours start with exact matching boundaries.
        </div>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            ...
            <div style={{ display: "grid", gap: 6, padding: "9px 10px", borderRadius: 9, background: "rgba(234,179,8,0.06)", border: "1px solid rgba(234,179,8,0.18)" }}>
              ...
              <div style={{ fontSize: 10.6, color: "rgba(255,255,255,0.52)", lineHeight: 1.4 }}>
                Explicit top-level <code>polities</code> metadata (<code>flag</code>/<code>flagCode</code>) is imported first. When it is absent, this option may fill safely recognized owners from OpenHistoria&apos;s built-in catalog. Existing custom/historical campaign flags are preserved.
              </div>
            </div>
