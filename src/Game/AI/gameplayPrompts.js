...
- install makes the puppet state; kind says which powers the overlord holds (protectorate: its foreign policy; satellite: real independence behind a sovereign front; client: a bought or installed government); secrecy is open if the world knows, covert if only the two parties do; loyalty 0-100 is how far the puppet accepts it. reclassify changes kind, loyalty moves the score, reveal makes a covert one open for good, release/annex/revolt end it, suppress puts down a rising. Every field is required; for an op that does not use one, repeat the arrangement's current value.
