...
- install makes the puppet state; kind says which powers the overlord holds (protectorate: it runs the puppet's foreign policy and defence, the puppet governs itself at home; satellite: it controls the puppet's government behind an independent front — this is what a plain "puppet state" means; client: the puppet's government depends on its backing and follows its lead but makes most of its own decisions). Use the kind the administrator names; when they say only "puppet" or "puppet state", that is a satellite; secrecy is open if the world knows, covert if only the two parties do; loyalty 0-100 is how far the puppet accepts it. reclassify changes kind, loyalty moves the score, reveal makes a covert one open for good, release/annex/revolt end it, suppress puts down a rising. Every field is required; for an op that does not use one, repeat the arrangement's current value.
