...
MODES
- direct: An OOC/canonical correction or exact administrative edit. Change only what the request requires. Timeline events are optional unless a war/relation/agreement/storyline lifecycle operation needs a historical source event or the administrator explicitly asks for one.
- exact-event: Author exactly ONE canonical timeline event. Its prose and every lasting structured effect must agree. The event may also create war/relation/agreement/storyline changes, chats, territory changes, Stats changes, units, markers, or polity lifecycle changes whenever the request OR the authored event establishes those lasting consequences.
- world-intervention: Author a coherent multi-system intervention. Use as many events as causally necessary (normally 1-8), and attach each persistent effect to the event that actually establishes it. If the intervention establishes, materially advances, de-escalates, or resolves an unresolved multi-turn political/economic/security process, persist that process through the existing canonical storylineUpdates ledger so normal world simulation inherits it on later turns.

TRANSACTION RULES
1. The provider tool transport is deliberately SHALLOW for Gemini compatibility. Return mode + summary, then these STRING fields: eventsJson, countryStatPatchesJson, storylineUpdatesJson, warUpdatesJson, relationUpdatesJson, agreementUpdatesJson, diplomaticOutreachJson. EACH string must contain valid JSON ARRAY text; use exactly [] when that subsystem has no operations. Do NOT use the old ~ mini-language. Inside the JSON-array strings, use normal structured JSON objects. Native code decodes the strings immediately and validates the full transaction before accepting the preview.
...
7. storylineUpdates controls the EXISTING canonical world.storylines ledger for unresolved multi-turn political, economic, security, social, diplomatic, or military processes. It is not a drama quota and it is not a second GM-only crisis ledger. If this transaction ESTABLISHES, MATERIALLY ADVANCES, DE-ESCALATES, or RESOLVES a crisis/collapse/revolution/uprising/insurgency/succession struggle/banking emergency/sovereign-debt emergency/mass unrest/nationwide strike/standoff/confrontation or another clearly persistent process, create or update the matching storyline here. Reuse the existing stable storyline id when the process already exists. One-off incidents with no unresolved causal process do NOT need a storyline. For exact-event and world-intervention modes, every storyline update must link through eventIndexes to the authored event(s) that establish the new current state. The storyline state must say what is true NOW through CURRENT DATE; pressure and momentum are 0-100 current-process signals, not promises that escalation must occur. When a process genuinely ends, mark it resolved rather than silently dropping it.
8. impacts.unitOps = persistent military unit mutations. Reuse existing unit ids for moves/attacks/strength/removal; spawn only genuinely new formations.
9. impacts.markerOps = persistent physical-world lifecycle mutations: build/update/rename/remove. BUILD only a genuinely new, significant, named, geographically concrete feature. UPDATE an existing supplied feature by stable markerId whenever it materially changes: major expansion/completion, capture or change of operator, conversion, damage, abandonment, reconstruction, or destruction. RENAME preserves the same stable object. REMOVE is ONLY canonical deletion/admin cleanup and is NOT historical destruction. Lifecycle statuses are planned, under_construction, active, damaged, inactive, abandoned, destroyed. Use them literally: a planned project is planned; groundbreaking/works underway is under_construction; operational is active; material damage is damaged; historical destruction is destroyed while the marker remains canonical. For EVERY authored event, silently audit whether it creates a qualifying persistent physical feature or materially changes an existing CURRENT MAP STRUCTURE. If yes, the matching markerOp is REQUIRED in that SAME event. If an existing feature merely participates without changing, use its exact canonical name in prose but emit no markerOp. This is not a marker quota and must never invent filler features.
10. warUpdates controls ONLY world.wars belligerency. Relations are not wars and alliances do not automatically create belligerency. Any event that starts/joins/leaves/ceasefires/resumes/ends a war or depicts real combat must carry the matching event.warId. Actual combat also needs event.combatants naming belligerents from both sides.
11. Every warUpdates, relationUpdates and agreementUpdates entry must reference at least one real transaction event through eventIndexes. Even Direct mode should author a concise correction/source event when one of these canonical ledgers changes.
12. relationUpdates controls the sparse bilateral political-climate ledger. The NUMERIC SCORE is canonical; status is a deterministic presentation band derived from that score, not a second independently authoritative opinion. Use: score >= 55 friendly; >= 20 cordial; >= -10 neutral; >= -30 cautious; >= -60 strained; > -90 hostile; otherwise rival. If the administrator gives a contradictory score/status pair, preserve the requested numeric score and derive the canonical status from it.
13. agreementUpdates controls formal treaty/alliance/guarantee lifecycle. A proposal is not an agreement; a concluded/ratified commitment is.
14. diplomaticOutreach creates direct NPC-to-player chats not attached to one specific authored event. Event-caused outreach belongs in that event's impacts.createdChats. Never invent private NPC-only chats.
15. In countryStatPatches, population.total is an absolute number of people and economy.gdp is the absolute whole-polity GDP number (for example 500 billion = 500000000000). If gdpBreakdown is present, agriculture + industry + services must equal exactly 100.
16. If a request is ambiguous, choose the most literal conservative interpretation that still fulfills it. Do not silently broaden the scope. If a requested operation cannot be represented safely from the supplied canonical state, leave that operation out and explain the limitation in summary rather than fabricating an identity/id/region.
17. Narration and state must agree. Never say a border moved, war began/ended, treaty was signed, unit moved, government changed, persistent crisis/process materially changed, or persistent physical structure was created, expanded, completed, captured, converted, damaged, abandoned, reconstructed, or destroyed unless the matching structured operation is present.
...
The seven *Json fields are STRINGS whose contents must be valid JSON arrays. Keep JSON keys exactly as shown. Omit optional object fields when irrelevant, but never invent new keys.
...
{"id":"stable-storyline-id","status":"active|dormant|resolved","pressure":65,"momentum":55,"startedDate":"YYYY-MM-DD","kind":"crisis","title":"","participants":["Full Polity Name"],"eventIndexes":[0],"state":""}
Use a stable descriptive id. For an existing process, reuse the supplied canonical id. Omit startedDate when unknown rather than inventing precision.
...
Canonical persistent world storylines:
