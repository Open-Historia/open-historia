...
const DecisionCard = ({ proposal }) => {
  ...
  return <div data-institution-decision-card="formal" style={{ ...panel, padding: ".8rem", background: "rgba(255,255,255,.026)" }}>
    ...
    {ballots.length > 0 && <div style={{ marginTop: ".65rem", paddingTop: ".5rem", borderTop: "1px solid rgba(255,255,255,.06)" }}>
      <button type="button" onClick={() => setPositionsOpen((open) => !open)} aria-expanded={positionsOpen} data-institution-recorded-positions-toggle="true" style={{ display: "inline-flex", alignItems: "center", gap: ".35rem", border: 0, background: "transparent", color: "#c4b5fd", padding: 0, cursor: "pointer", fontSize: ".58rem", fontWeight: 760 }}>
...
const LifecycleCaseCard = ({ entry, institutionName = "", canRespond = false, canOpenNegotiation = false, busy = false, opening = false, onRespond = null, onOpenNegotiation = null }) => {
  ...
  return <div data-institution-lifecycle-case={clean(entry?.id)} style={{ ...panel, padding: ".6rem .65rem", background: "rgba(139,92,246,.035)" }}>
    ...
    {canOpenNegotiation && <div data-foreign-lifecycle-negotiation-controls="true" style={{ display: "flex", justifyContent: "flex-end", marginTop: ".5rem", paddingTop: ".45rem", borderTop: "1px solid rgba(255,255,255,.06)" }}>
      <button disabled={busy} onClick={() => onOpenNegotiation?.()} style={{ border: "1px solid rgba(167,139,250,.24)", borderRadius: 7, background: "rgba(139,92,246,.09)", color: "#ddd6fe", padding: ".28rem .46rem", fontSize: ".54rem", fontWeight: 760, cursor: busy ? "wait" : "pointer" }}>{opening ? "Opening…" : "Open negotiation →"}</button>
    </div>}
    {canRespond && <div data-player-lifecycle-response-controls="true" style={{ display: "flex", flexWrap: "wrap", gap: ".3rem", marginTop: ".5rem", paddingTop: ".45rem", borderTop: "1px solid rgba(255,255,255,.06)" }}>
      <button disabled={busy} onClick={() => onRespond?.("accept")} style={{ border: "1px solid rgba(34,197,94,.24)", borderRadius: 7, background: "rgba(34,197,94,.08)", color: "#bbf7d0", padding: ".26rem .42rem", fontSize: ".54rem", fontWeight: 760, cursor: busy ? "wait" : "pointer" }}>Accept</button>
      <button disabled={busy} onClick={() => onRespond?.("seek-observer")} style={{ border: "1px solid rgba(167,139,250,.22)", borderRadius: 7, background: "rgba(139,92,246,.08)", color: "#ddd6fe", padding: ".26rem .42rem", fontSize: ".54rem", cursor: busy ? "wait" : "pointer" }}>Seek observer status</button>
      <button disabled={busy} onClick={() => onRespond?.("delay")} style={{ border: "1px solid rgba(245,158,11,.2)", borderRadius: 7, background: "rgba(245,158,11,.06)", color: "#fde68a", padding: ".26rem .42rem", fontSize: ".54rem", cursor: busy ? "wait" : "pointer" }}>Decide later</button>
      <button disabled={busy} onClick={() => onRespond?.("reject")} style={{ border: "1px solid rgba(239,68,68,.2)", borderRadius: 7, background: "rgba(239,68,68,.06)", color: "#fca5a5", padding: ".26rem .42rem", fontSize: ".54rem", cursor: busy ? "wait" : "pointer" }}>Reject</button>
    </div>}
  </div>;
};
  ...
  return <div data-polity-multi-picker="true" style={{ position: "relative" }}>
    <div style={{ display: "flex", flexWrap: "wrap", gap: ".3rem", minHeight: "2.15rem", alignItems: "center", border: "1px solid rgba(255,255,255,.1)", borderRadius: 8, background: "rgba(8,8,12,.72)", padding: ".28rem .36rem" }}>
      {selected.map((polity) => <span key={polity} data-selected-polity={polity} style={{ display: "inline-flex", alignItems: "center", gap: ".28rem", maxWidth: "15rem", border: "1px solid rgba(167,139,250,.25)", borderRadius: 999, background: "rgba(139,92,246,.12)", color: "#ede9fe", padding: ".22rem .38rem .22rem .46rem", fontSize: ".55rem", fontWeight: 720 }}>
        ...
        <button type="button" aria-label={`Remove ${polity}`} onClick={() => remove(polity)} style={{ border: 0, background: "transparent", color: "rgba(255,255,255,.5)", padding: 0, cursor: "pointer", fontSize: ".7rem", lineHeight: 1 }}>×</button>
      </span>)}
      <input
        value={query}
...
const FoundInstitutionForm = ({ value, onChange, onCancel, onSubmit, busy, polities = [] }) => {
  ...
  return <div data-institution-founding-wizard="true" style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: ".8rem 1rem 1rem" }}>
    <div style={{ maxWidth: "54rem", margin: "0 auto", display: "flex", flexDirection: "column", gap: ".65rem" }}>
      ...
      <div style={{ ...panel, padding: ".75rem", display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: ".55rem .65rem" }}>
        <label style={{ gridColumn: "1 / -1", fontSize: ".56rem", color: "rgba(255,255,255,.44)" }}>Name<input autoFocus value={value.name} onChange={(e) => set("name", e.target.value)} placeholder="Baltic Union" maxLength={160} style={{ ...fieldStyle, marginTop: ".22rem" }} /></label>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: ".45rem" }}>
        <button type="button" disabled={busy} onClick={onCancel} style={{ border: "1px solid rgba(255,255,255,.1)", borderRadius: 8, background: "rgba(255,255,255,.035)", color: "rgba(255,255,255,.65)", padding: ".42rem .7rem", fontSize: ".62rem", cursor: busy ? "wait" : "pointer" }}>Cancel</button>
        <button type="button" disabled={busy || !clean(value.name)} onClick={onSubmit} style={{ border: "1px solid rgba(167,139,250,.32)", borderRadius: 8, background: "rgba(139,92,246,.16)", color: "#ede9fe", padding: ".42rem .75rem", fontSize: ".62rem", fontWeight: 780, cursor: busy || !clean(value.name) ? "not-allowed" : "pointer" }}>{busy ? "Founding…" : "Found institution"}</button>
      </div>
    </div>
  </div>;
};

export default function InstitutionsWorkspace({ world = {}, playerCountry = "", gameDate = "", chats = [], unreadIds = new Set(), onAdoptResult, onOpenCouncil, onOpenLifecycleChat, onCouncilVisibleChange, renderCouncil, focusRequest = null }) {
  ...
  if (!selectedView) {
    return <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <div style={{ padding: ".55rem 1rem", borderBottom: "1px solid rgba(255,255,255,.06)", display: "flex", alignItems: "center", gap: ".45rem", flexShrink: 0 }}>
        <button onClick={() => setShowAll(false)} style={{ border: "1px solid rgba(255,255,255,.1)", borderRadius: 999, background: !showAll ? "rgba(139,92,246,.14)" : "rgba(255,255,255,.03)", color: !showAll ? "#ddd6fe" : "rgba(255,255,255,.55)", padding: ".28rem .55rem", fontSize: ".62rem", cursor: "pointer" }}>Your institutions ({personalRows.length})</button>
        ...
        <button type="button" onClick={() => { setFoundingOpen(true); setError(""); }} style={{ marginLeft: "auto", border: "1px solid rgba(167,139,250,.3)", borderRadius: 8, background: "rgba(139,92,246,.14)", color: "#ede9fe", padding: ".32rem .58rem", fontSize: ".6rem", fontWeight: 780, cursor: "pointer" }}>＋ Found institution</button>
      </div>
  ...
  return <div data-institution-shell="persistent" style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
    ...
    <div data-institution-tabs="persistent" style={{ display: "flex", flexWrap: "wrap", gap: ".15rem", padding: "0 .7rem", borderBottom: "1px solid rgba(255,255,255,.06)", flexShrink: 0 }}>{tabs.map(([key, label]) => <button key={key} onClick={() => key === "council" && selectedRow.member ? openCouncil() : setSection(key)} disabled={key === "council" && Boolean(busy)} style={{ flex: "0 0 auto", border: 0, borderBottom: `2px solid ${section === key ? "rgba(139,92,246,.9)" : "transparent"}`, background: "transparent", color: section === key ? "#ede9fe" : "rgba(255,255,255,.47)", padding: ".48rem .55rem .55rem", fontSize: ".61rem", fontWeight: 720, cursor: key === "council" && busy ? "wait" : "pointer" }}>{key === "council" && busy === "council" ? "Opening…" : label}</button>)}</div>
    {error && <div style={{ margin: ".5rem 1rem 0", padding: ".45rem .6rem", borderRadius: 8, background: "rgba(239,68,68,.08)", border: "1px solid rgba(239,68,68,.2)", color: "#fca5a5", fontSize: ".62rem", flexShrink: 0 }}>{error}</div>}
    {section === "council" && selectedPendingActions > 0 && <button type="button" onClick={() => setSection("agenda")} style={{ margin: ".55rem 1rem 0", padding: ".58rem .7rem", display: "flex", alignItems: "center", gap: ".7rem", border: "1px solid rgba(245,158,11,.28)", borderRadius: 10, background: "rgba(245,158,11,.08)", color: "#fde68a", cursor: "pointer", textAlign: "left", flexShrink: 0 }}>
      ...
      {section === "overview" && <div style={{ display: "flex", flexDirection: "column", gap: ".6rem" }}>
        ...
        {!selectedRow.member && lower(institution.status) !== "dissolved" && <div data-institution-accession-controls="true" style={{ ...panel, padding: ".72rem", borderColor: "rgba(167,139,250,.16)" }}>
          ...
          <div style={{ display: "flex", gap: ".4rem", marginTop: ".45rem", flexWrap: "wrap" }}>
            ...
            {list(institution?.charter?.lifecycle?.accession?.allowedStatuses).includes("observer") && <button disabled={Boolean(busy) || selectedPendingLifecycle.some((entry) => lower(entry.polity) === lower(playerCountry))} onClick={() => applyForMembership("observer")} style={{ border: "1px solid rgba(255,255,255,.1)", borderRadius: 8, background: "rgba(255,255,255,.04)", color: "rgba(255,255,255,.7)", padding: ".34rem .55rem", fontSize: ".58rem", cursor: busy ? "wait" : "pointer" }}>Request observer status</button>}
          </div>
        </div>}
        ...
        {selectedRow.member && selectedView.canParticipate && lower(institution.status) !== "dissolved" && <div data-institution-lifecycle-controls="true" style={{ ...panel, padding: ".72rem" }}>
          ...
          <div style={{ marginTop: ".45rem", display: "grid", gridTemplateColumns: "minmax(12rem,1fr) auto", gap: ".4rem" }}>
            ...
            <button disabled={Boolean(busy) || !clean(inviteTarget)} onClick={invitePolity} style={{ border: "1px solid rgba(167,139,250,.25)", borderRadius: 8, background: "rgba(139,92,246,.11)", color: "#ddd6fe", padding: ".35rem .55rem", fontSize: ".58rem", fontWeight: 750, cursor: busy || !clean(inviteTarget) ? "not-allowed" : "pointer" }}>Send invitation</button>
          </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: ".35rem", marginTop: ".4rem" }}>
            <button disabled={Boolean(busy)} onClick={withdraw} style={{ border: "1px solid rgba(245,158,11,.2)", borderRadius: 8, background: "rgba(245,158,11,.07)", color: "#fde68a", padding: ".32rem .5rem", fontSize: ".56rem", cursor: busy ? "wait" : "pointer" }}>Withdraw from institution</button>
            <button disabled={Boolean(busy)} onClick={() => proposeLifecycle("dissolve")} style={{ border: "1px solid rgba(239,68,68,.2)", borderRadius: 8, background: "rgba(239,68,68,.06)", color: "#fca5a5", padding: ".32rem .5rem", fontSize: ".56rem", cursor: busy ? "wait" : "pointer" }}>Propose dissolution</button>
          </div>
        </div>}
      ...
      {section === "members" && <div style={{ display: "flex", flexDirection: "column", gap: ".5rem" }}>
        ...
        {selectedView.members?.map((member) => <div key={member.polity} style={{ ...panel, padding: ".55rem .65rem", display: "flex", alignItems: "center", gap: ".5rem" }}>
          ...
          {selectedView.canParticipate && lower(member.polity) !== lower(playerCountry) && lower(institution.status) !== "dissolved" && <div style={{ display: "flex", gap: ".25rem" }}>
            {lower(member.status) === "suspended" ? <button disabled={Boolean(busy)} onClick={() => proposeLifecycle("reinstate", member.polity)} style={{ border: "1px solid rgba(34,197,94,.18)", borderRadius: 7, background: "rgba(34,197,94,.06)", color: "#86efac", padding: ".22rem .38rem", fontSize: ".52rem", cursor: busy ? "wait" : "pointer" }}>Propose reinstate</button> : <button disabled={Boolean(busy)} onClick={() => proposeLifecycle("suspend", member.polity)} style={{ border: "1px solid rgba(245,158,11,.18)", borderRadius: 7, background: "rgba(245,158,11,.06)", color: "#fde68a", padding: ".22rem .38rem", fontSize: ".52rem", cursor: busy ? "wait" : "pointer" }}>Propose suspend</button>}
            <button disabled={Boolean(busy)} onClick={() => proposeLifecycle("expel", member.polity)} style={{ border: "1px solid rgba(239,68,68,.18)", borderRadius: 7, background: "rgba(239,68,68,.06)", color: "#fca5a5", padding: ".22rem .38rem", fontSize: ".52rem", cursor: busy ? "wait" : "pointer" }}>Propose expel</button>
          </div>}
        </div>)}
