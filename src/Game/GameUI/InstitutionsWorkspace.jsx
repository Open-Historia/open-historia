...
export const Emblem = ({ institution, size = 42 }) => {
  ...
  return <div style={{ width: size, height: size, flex: "0 0 auto", borderRadius: 11, display: "grid", placeItems: "center", overflow: "hidden", padding: url && !failed ? 4 : 0, background: "var(--oh-grey-raised)", border: "1px solid var(--oh-grey-border)" }} title={institution?.name || institution?.id || "Institution"}>
    {url && !failed ? <img src={url} alt="" aria-hidden="true" onError={() => setFailed(true)} style={{ width: "100%", height: "100%", objectFit: "contain", borderRadius: 7 }} /> : <span style={{ fontSize: ".64rem", fontWeight: 850, color: "var(--oh-grey-text)" }}>{mark || "IN"}</span>}
  </div>;
};
...
const InstitutionRow = ({ row, selected, unread = false, onClick }) => {
  ...
  return <button className="oh-tap-row" type="button" onClick={onClick} style={{ ...panel, width: "100%", display: "flex", gap: ".7rem", alignItems: "center", textAlign: "left", padding: ".7rem", cursor: "pointer", color: "white", borderColor: selected ? "var(--oh-grey-border-strong)" : "rgba(255,255,255,.08)", background: selected ? "var(--oh-grey-selected)" : "rgba(255,255,255,.03)" }}>
  ...
  return <div data-institution-decision-card="formal" style={{ ...panel, padding: ".8rem", background: "rgba(255,255,255,.026)" }}>
    <div style={{ display: "flex", alignItems: "flex-start", gap: ".6rem" }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: ".59rem", fontWeight: 850, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--oh-grey-muted)" }}>Formal decision</div>
    ...
    {ballots.length > 0 && <div style={{ marginTop: ".65rem", paddingTop: ".5rem", borderTop: "1px solid rgba(255,255,255,.06)" }}>
      <button className="oh-tap-row" type="button" onClick={() => setPositionsOpen((open) => !open)} aria-expanded={positionsOpen} data-institution-recorded-positions-toggle="true" style={{ display: "inline-flex", alignItems: "center", gap: ".35rem", border: 0, background: "transparent", color: "rgba(231,231,234,.72)", padding: 0, cursor: "pointer", fontSize: ".62rem", fontWeight: 760 }}>        <span>{positionsOpen ? "Hide" : "View"} recorded member positions</span>
        <SmallPill tone="neutral">{ballots.length}</SmallPill>
...
const LifecycleCaseCard = ({ entry, institutionName = "", canRespond = false, canOpenNegotiation = false, busy = false, opening = false, onRespond = null, onOpenNegotiation = null }) => {
  ...
  return <div data-institution-lifecycle-case={clean(entry?.id)} style={{ ...panel, padding: ".6rem .65rem", background: "rgba(255,255,255,.025)" }}>
...
const LifecycleRuleRow = ({ label, rule }) => <div style={{ ...panel, padding: ".58rem .65rem", display: "flex", alignItems: "center", gap: ".55rem" }}>
  ...
  <SmallPill tone="neutral">{humanize(rule?.mode || "approval")}</SmallPill>
...
const PolityMultiPicker = ({ value = "", onChange, polities = [], label = "Select governments", multiple = true }) => {
  ...
  return <div data-polity-multi-picker="true" style={{ position: "relative" }}>
    <div style={{ display: "flex", flexWrap: "wrap", gap: ".3rem", minHeight: "2.15rem", alignItems: "center", border: "1px solid rgba(255,255,255,.1)", borderRadius: 8, background: "rgba(8,8,12,.72)", padding: ".28rem .36rem" }}>
      {selected.map((polity) => <span key={polity} data-selected-polity={polity} style={{ display: "inline-flex", alignItems: "center", gap: ".28rem", maxWidth: "15rem", border: "1px solid var(--oh-grey-border)", borderRadius: 999, background: "rgba(255,255,255,.055)", color: "var(--oh-grey-text)", padding: ".22rem .38rem .22rem .46rem", fontSize: ".6rem", fontWeight: 720 }}>
...
const FoundInstitutionForm = ({ value, onChange, onCancel, onSubmit, busy, polities = [] }) => {
  ...
  const sectionLabelStyle = { fontSize: ".62rem", fontWeight: 850, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--oh-grey-muted)" };
  ...
  return <div data-institution-founding-wizard="true" style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "1rem 1.15rem 1.2rem" }}>
    <div style={{ maxWidth: "50rem", margin: "0 auto", display: "flex", flexDirection: "column", gap: ".85rem" }}>      <div>
        <div style={{ fontSize: ".64rem", fontWeight: 850, letterSpacing: ".09em", textTransform: "uppercase", color: "var(--oh-grey-muted)" }}>Found an institution</div>
      ...
      <div style={{ ...panel, padding: ".9rem", display: "grid", gridTemplateColumns: isMobile ? "minmax(0,1fr)" : "repeat(2,minmax(0,1fr))", gap: ".75rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "end", justifyContent: "flex-end" }}><button className="oh-tap-row" type="button" onClick={() => setAdvanced((open) => !open)} style={{ border: advanced ? "1px solid var(--oh-grey-border-strong)" : "1px solid var(--oh-grey-border)", borderRadius: 8, background: advanced ? "var(--oh-grey-selected)" : "rgba(255,255,255,.035)", color: advanced ? "var(--oh-grey-text)" : "rgba(255,255,255,.68)", padding: ".48rem .7rem", fontSize: ".64rem", fontWeight: 760, cursor: "pointer" }}>{advanced ? "Hide advanced settings" : "Advanced settings"}</button></div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: ".6rem", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: ".45rem" }}>
          ...
          <button className="oh-tap-row" type="button" disabled={busy || !clean(value.name)} onClick={onSubmit} style={{ border: "1px solid var(--oh-grey-border-strong)", borderRadius: 8, background: "var(--oh-grey-raised)", color: "var(--oh-grey-text)", padding: ".46rem .8rem", fontSize: ".66rem", fontWeight: 780, cursor: busy || !clean(value.name) ? "not-allowed" : "pointer" }}>{busy ? "Founding…" : "Found institution"}</button>
        </div>      </div>
    </div>
  </div>;
};
...
export default function InstitutionsWorkspace({ panelOpen = true, world = {}, playerCountry = "", gameDate = "", chats = [], unreadIds = new Set(), onAdoptResult, onOpenCouncil, onOpenLifecycleChat, onRequestCouncilTurn, councilAutomation = [], onCouncilVisibleChange, renderCouncil, focusRequest = null }) {  const [showAll, setShowAll] = useState(false);
  ...
  const recentActivity = useMemo(() => {
    const formal = list(selectedView?.decisionHistory).map((proposal) => ({
      ...
      tone: ["passed", "implementation"].includes(lower(proposal.status)) ? "good" : ["failed", "vetoed"].includes(lower(proposal.status)) ? "bad" : "neutral",
    }));
  ...
  if (!selectedView) {
    return <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <div style={{ padding: ".55rem 1rem", borderBottom: "1px solid rgba(255,255,255,.06)", display: "flex", alignItems: "center", gap: ".45rem", flexShrink: 0 }}>
        <button className="oh-tap-row" onClick={() => setShowAll(false)} style={{ border: "1px solid rgba(255,255,255,.1)", borderRadius: 999, background: !showAll ? "var(--oh-grey-selected)" : "rgba(255,255,255,.03)", color: !showAll ? "var(--oh-grey-text)" : "rgba(255,255,255,.55)", padding: ".28rem .55rem", fontSize: ".62rem", cursor: "pointer" }}>Your institutions ({personalRows.length})</button>
        <button className="oh-tap-row" onClick={() => setShowAll(true)} style={{ border: "1px solid rgba(255,255,255,.1)", borderRadius: 999, background: showAll ? "var(--oh-grey-selected)" : "rgba(255,255,255,.03)", color: showAll ? "var(--oh-grey-text)" : "rgba(255,255,255,.55)", padding: ".28rem .55rem", fontSize: ".62rem", cursor: "pointer" }}>View all ({allRows.length})</button>
        {pending > 0 && <SmallPill tone="live">{pending} awaiting you</SmallPill>}
        <button className="oh-tap-row" type="button" onClick={() => { setFoundingOpen(true); setError(""); }} style={{ marginLeft: "auto", border: "1px solid var(--oh-grey-border-strong)", borderRadius: 8, background: "var(--oh-grey-raised)", color: "var(--oh-grey-text)", padding: ".32rem .58rem", fontSize: ".64rem", fontWeight: 780, cursor: "pointer" }}>＋ Found institution</button>      </div>
  ...
  return <div data-institution-shell="persistent" style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
    ...
    <div data-institution-tabs="persistent" style={{ display: "flex", flexWrap: "wrap", gap: ".08rem", padding: "0 .78rem", borderBottom: "1px solid rgba(255,255,255,.06)", flexShrink: 0 }}>{tabs.map(([key, label]) => <button className="oh-tap-row" key={key} onClick={() => key === "council" && selectedRow.member ? openCouncil() : setSection(key)} disabled={key === "council" && Boolean(busy)} style={{ flex: "0 0 auto", border: 0, borderBottom: `2px solid ${section === key ? "rgba(231,231,234,.72)" : "transparent"}`, background: "transparent", color: section === key ? "var(--oh-grey-text)" : "rgba(255,255,255,.52)", padding: ".58rem .68rem .62rem", fontSize: ".68rem", fontWeight: 730, cursor: key === "council" && busy ? "wait" : "pointer" }}>{key === "council" && busy === "council" ? "Opening…" : label}</button>)}</div>    {error && <div style={{ margin: ".5rem 1rem 0", padding: ".45rem .6rem", borderRadius: 8, background: "rgba(239,68,68,.08)", border: "1px solid rgba(239,68,68,.2)", color: "#fca5a5", fontSize: ".62rem", flexShrink: 0 }}>{error}</div>}
    ...
    <div data-institution-content={section} style={{ flex: 1, minHeight: 0, overflowY: section === "council" ? "hidden" : "auto", padding: section === "council" ? ".55rem 1rem .8rem" : ".8rem 1rem 1rem", display: section === "council" ? "flex" : "block", flexDirection: section === "council" ? "column" : undefined }}>
      {section === "overview" && <div data-institution-overview="focused" style={{ display: "flex", flexDirection: "column", gap: ".75rem", maxWidth: "56rem", margin: "0 auto" }}>
        <div data-institution-current-business="true" style={{ ...panel, padding: "1rem", borderColor: selectedView.activeProposals?.length || selectedPendingLifecycle.length ? "var(--oh-grey-border-strong)" : "rgba(255,255,255,.075)", background: "rgba(255,255,255,.025)" }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: ".75rem" }}>
            <div style={{ width: "2.25rem", height: "2.25rem", borderRadius: 10, display: "grid", placeItems: "center", background: "var(--oh-grey-raised)", border: "1px solid var(--oh-grey-border)", color: "rgba(231,231,234,.72)", fontSize: "1rem" }} aria-hidden="true">▤</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: ".65rem", fontWeight: 850, letterSpacing: ".075em", textTransform: "uppercase", color: "var(--oh-grey-muted)" }}>Current business</div>
              {selectedView.activeProposals?.length ? <>
                ...
                <div style={{ marginTop: ".48rem", display: "flex", gap: ".35rem", flexWrap: "wrap" }}><SmallPill tone={selectedView.activeProposals[0].status === "voting" ? "live" : "neutral"}>{humanize(selectedView.activeProposals[0].status)}</SmallPill>{selectedView.activeProposals.length > 1 && <SmallPill tone="neutral">+{selectedView.activeProposals.length - 1} more</SmallPill>}</div>
          ...
          <div style={{ display: "flex", gap: ".45rem", flexWrap: "wrap", marginTop: ".8rem", paddingTop: ".72rem", borderTop: "1px solid rgba(255,255,255,.06)" }}>
            {selectedView.canTableProposal && <button type="button" onClick={() => setSection("agenda")} style={{ border: "1px solid var(--oh-grey-border-strong)", borderRadius: 9, background: "var(--oh-grey-raised)", color: "var(--oh-grey-text)", padding: ".46rem .7rem", fontSize: ".67rem", fontWeight: 780, cursor: "pointer" }}>＋ Table a matter</button>}
            ...
            {selectedPendingLifecycle.length > 0 && <button type="button" onClick={() => setSection("members")} style={{ marginLeft: "auto", border: 0, background: "transparent", color: "rgba(231,231,234,.72)", padding: ".46rem .2rem", fontSize: ".65rem", fontWeight: 730, cursor: "pointer" }}>Review membership →</button>}
          </div>
        </div>

        <div data-institution-recent-activity="true" style={{ ...panel, padding: ".9rem 1rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: ".55rem" }}><strong style={{ flex: 1, fontSize: ".78rem" }}>Recent activity</strong>{(selectedView.decisionHistory?.length || selectedHistory.length) > recentActivity.length && <button type="button" onClick={() => setSection("decisions")} style={{ border: 0, background: "transparent", color: "rgba(231,231,234,.72)", fontSize: ".62rem", cursor: "pointer" }}>View formal record →</button>}</div>
          {recentActivity.length ? <div style={{ marginTop: ".65rem", display: "flex", flexDirection: "column" }}>{recentActivity.map((entry, index) => <div key={entry.id} style={{ display: "grid", gridTemplateColumns: "auto minmax(0,1fr) auto", gap: ".6rem", alignItems: "start", padding: ".48rem 0", borderTop: index ? "1px solid rgba(255,255,255,.055)" : 0 }}>
            <span style={{ width: ".5rem", height: ".5rem", marginTop: ".28rem", borderRadius: 999, background: entry.tone === "good" ? "#4ade80" : entry.tone === "bad" ? "#f87171" : "rgba(255,255,255,.35)" }} />
      ...
      {section === "agenda" && <div style={{ display: "flex", flexDirection: "column", gap: ".55rem" }}>
        {selectedView.canTableProposal && <div data-institution-table-proposal="true" style={{ ...panel, padding: ".85rem" }}>
          ...
          <div style={{ display: "flex", gap: ".42rem", marginTop: ".55rem", flexWrap: "wrap" }}>
            <button disabled={Boolean(busy) || !proposalTitle.trim()} onClick={() => createProposal("debate")} style={{ border: "1px solid var(--oh-grey-border-strong)", borderRadius: 8, background: "var(--oh-grey-raised)", color: "var(--oh-grey-text)", cursor: busy || !proposalTitle.trim() ? "not-allowed" : "pointer", padding: ".42rem .64rem", fontSize: ".64rem", fontWeight: 780 }}>Open for debate</button>
      ...
      {section === "decisions" && <div style={{ display: "flex", flexDirection: "column", gap: ".55rem" }}>
        <div style={{ padding: ".05rem .1rem .2rem" }}>
          <div style={{ fontSize: ".59rem", fontWeight: 850, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--oh-grey-muted)" }}>Institutional record</div>
