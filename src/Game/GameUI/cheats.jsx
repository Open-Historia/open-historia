...
const TOOLS = [
    ...
    { id: "difficulty", title: "Difficulty", subtitle: "Tune simulation rigor without anti-player bias", icon: "◈", badge: "2.0" },
    ...
    { id: "regions", title: "Region Inspector", subtitle: "Inspect control, sovereignty, claims, and region identity", icon: "▦", badge: "2.0" },
    { id: "edit-feature", title: "Map Feature Editor", subtitle: "Inspect and edit runtime features and scenario cities", icon: "◉", badge: "2.0" },
    { id: "add-feature", title: "Add Map Feature", subtitle: "Place cities, HQs, landmarks, ports, and other world features", icon: "+", badge: "2.0" },
    ...
    { id: "events", title: "Event Editor", subtitle: "Search, create, and repair canonical timeline events", icon: "≡", badge: "2.0" },
];
...
const CheatsPanel = ({ open, onClose, onOpenForces }) => {
    ...
    return (
        ...
        {!tool ? (
            <>
            {header("Cheats 2.0", "Game Master, world editing, and simulation administration") }
...
const EventEditorView = ({ meta, header, busy, status, game, runBusy }) => {
    ...
    useEffect(() => {
        ...
        load()
            ...
            .catch((error) => {
                console.warn("[cheats 8A.4] event/timeline load failed:", error);
...
const ToolView = ({ tool, header, busy, status, game, polities, refresh, runBusy, beginClickMode, endClickMode, setStatus, navigateTool }) => {
    ...
    const applyAdminMarkerOps = async (markerOps) => {
        ...
        const result = applyEventImpactsToWorld({
            ...
            events: [{
                ...
                description: "Structured administrative mutation from Cheats 2.0 Map Feature Editor.",
                importance: "minor",
                kind: "world",
                notable: false,
                playerRelated: false,
    ...
    if (tool === "regions") {
        ...
        const controller = String(fields.controller ?? "").trim();
        ...
        const normalizeClaimants = (raw, currentController = "") => [...new Set(
            (Array.isArray(raw)
                ? raw
                ...
                .filter((value) => value && value !== currentController),
        )];
        ...
        const readRegionState = async ({ id, rawProps = null, fallback = fields } = {}) => {
            ...
            const currentController = Object.prototype.hasOwnProperty.call(ownership, resolvedId)
            ...
            const currentSovereign = Object.prototype.hasOwnProperty.call(sovereignty, resolvedId)
                ...
                : currentController;
            const currentClaimants = normalizeClaimants(claims[resolvedId], currentController);
            ...
            const next = {
                ...
                controllerTarget: currentController,
        ...
        const applyTerritoryImpacts = async (impacts, message) => {
            ...
            const result = applyEventImpactsToWorld({
                ...
                events: [{
                    ...
                    description: "Structured administrative mutation from Cheats 2.0 Region Inspector.",
                    importance: "minor",
                    kind: "world",
                    notable: false,
                    playerRelated: false,
        ...
        return (
            ...
            <div style={{ overflowY: "auto", paddingRight: "0.08rem" }}>
                <div style={{ background: "rgba(124,58,237,0.1)", border: "1px solid rgba(139,92,246,0.34)", borderRadius: 10, color: "rgba(233,213,255,0.88)", fontSize: "0.68rem", lineHeight: 1.45, padding: "0.58rem 0.68rem" }}>
                    <strong style={{ color: "#e9d5ff" }}>Canonical now · evolvable later.</strong> Region edits change the present territorial state only. Future war, occupation, liberation, treaties, or annexations remain free to change it naturally.
                </div>
                ...
                {regionId && (
                    ...
                    <div style={{ ...editorFieldStyle, marginTop: "0.55rem", padding: "0.58rem 0.65rem" }}>
                        <div style={{ alignItems: "flex-start", display: "flex", gap: "0.65rem", justifyContent: "space-between" }}>
                            ...
                            <span style={{ background: `${stateTone}18`, border: `1px solid ${stateTone}55`, borderRadius: 999, color: stateTone, flexShrink: 0, fontSize: "0.58rem", fontWeight: 850, letterSpacing: "0.04em", padding: "0.25rem 0.42rem", textTransform: "uppercase" }}>
                                {stateLabel}
                            </span>
                        </div>
                    ...
                    <div style={{ ...editorFieldStyle, marginTop: "0.55rem" }}>
                        <div style={editorSectionLabelStyle}>Claims & contested state</div>
                        {claimants.length ? (
                            <div style={{ display: "flex", flexDirection: "column", gap: "0.32rem" }}>
                                {claimants.map((claimant) => (
                                    <div key={claimant} style={{ alignItems: "center", background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.2)", borderRadius: 8, display: "flex", gap: "0.45rem", justifyContent: "space-between", padding: "0.4rem 0.48rem" }}>
                                        ...
                                        <button
                                            ...
                                            onClick={() => runBusy(() => applyTerritoryImpacts({
                                                regionControlOps: [{
                                                    ...
                                                    clearAll: false,
                                                    note: "Region Inspector removes specific claimant",
                                                }],
                                            }, `${nameOf(claimant)} removed from active claimants.`))}
                            ...
                            <div style={{ color: "rgba(255,255,255,0.42)", fontSize: "0.7rem" }}>No active claimants or contenders.</div>
                        )}

                        <div style={{ display: "flex", gap: "0.4rem", marginTop: "0.5rem" }}>
                            <div style={{ flex: 1, minWidth: 0 }}>
                                <PolitySelect polities={polities} value={fields.claimantTarget ?? ""} onChange={(value) => setFields({ ...fields, claimantTarget: value })} placeholder="Add claimant…" />
                            </div>
                            <button
                                type="button"
                                disabled={busy || !fields.claimantTarget || fields.claimantTarget === controller || claimants.includes(fields.claimantTarget)}
                                onClick={() => runBusy(() => applyTerritoryImpacts({
                                    regionControlOps: [{
                                        op: "contest",
                                        regionId,
                                        regionName: fields.name || "",
                                        fromCode: controller,
                                        actorCode: fields.claimantTarget,
                                        note: "Region Inspector adds claimant/contender",
                                    }],
                                }, `${nameOf(fields.claimantTarget)} added as claimant/contender.`))}
                            ...
                            <button
                                ...
                                onClick={() => runBusy(() => applyTerritoryImpacts({
                                    regionControlOps: [{
                                        ...
                                        fromCode: controller,
                                        clearAll: true,
                                        note: "Region Inspector clears all removable claimants",
                                    }],
                                }, isOccupied
                                    ? "Removable claims cleared. The legal sovereign remains represented while foreign occupation continues."
                                    : "All claimants cleared."))}
                                style={{ ...buttonStyle, marginTop: "0.42rem", width: "100%" }}
                            >
                                Clear all removable claims
                            </button>
                        )}
                    </div>
                    ...
                    <details style={{ ...editorFieldStyle, marginTop: "0.55rem", padding: "0.5rem 0.6rem" }}>
                        ...
                        <div style={{ marginTop: "0.5rem" }}>
                            ...
                            <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)", color: "rgba(255,255,255,0.4)", fontSize: "0.62rem", lineHeight: 1.45, marginTop: "0.55rem", paddingTop: "0.5rem" }}>
                ...
                {!regionId && (
                    <div style={{ color: "rgba(255,255,255,0.45)", fontSize: "0.72rem", lineHeight: 1.45, marginTop: "0.55rem" }}>
                        Pick any map region to inspect its live controller, legal sovereign, claimant state, and geographic provenance.
                    </div>
                )}
                {statusLine}
            </div>
